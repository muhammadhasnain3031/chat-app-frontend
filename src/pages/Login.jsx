...
const { data } = await axios.post(
  'https://chat-app-backend-rose-omega-71.vercel.app/api/auth/login', 
  formData, 
  ...
  return (
    <div style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', background:'#1a1a2e' }}>
      <div style={{ background:'#16213e', padding:'40px', borderRadius:'16px', width:'100%', maxWidth:'380px', border:'1px solid #0f3460' }}>
        <h2 style={{ color:'#e94560', textAlign:'center', marginBottom:'8px', fontSize:'22px' }}>💬 ChatApp</h2>
        <p style={{ color:'#888', textAlign:'center', marginBottom:'28px', fontSize:'13px' }}>Welcome back!</p>
        ...
        <button onClick={handleSubmit}
          style={{ width:'100%', padding:'12px', background:'#e94560', color:'#fff', border:'none', borderRadius:'8px', fontSize:'15px', fontWeight:'600', cursor:'pointer', marginTop:'4px' }}>
          Login
        </button>
        <p style={{ textAlign:'center', marginTop:'18px', fontSize:'13px', color:'#888' }}>
          No account? <Link to="/register" style={{ color:'#e94560', fontWeight:'500' }}>Register</Link>
        </p>
      </div>
    </div>
  );
